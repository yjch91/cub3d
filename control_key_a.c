...
/*   Updated: 2020/12/28 02:24:53 by jayun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	control_key_a_x(t_info *info, int n, double vdir_x)
{
	if (n == 0)
...
static void	control_key_a_y(t_info *info, int n, double vdir_y)
{
	if (n == 0)
...
void		control_key_a(t_info *info)
{
	...
	n = info->map[(int)(info->pos_x - vdir_x * info->movespeed)]
		[(int)(info->pos_y)];
	control_key_a_x(info, n, vdir_x);
	n = info->map[(int)(info->pos_x)]
		[(int)(info->pos_y - vdir_y * info->movespeed)];
	control_key_a_y(info, n, vdir_y);
}
